...
class intacctSink(RecordSink):
    ...
    def __init__(self, target: PluginBase, stream_name: str, schema: Dict, key_properties: Optional[List[str]]) -> None:
        ...
        self.client = get_client(
            ...
            headers={'User-Agent': target.config['user_agent']} if 'user_agent' in target.config else {},
        )

        # Lookup for vendors
        vendors = self.client.get_entity(object_type='accounts_payable_vendors',fields=["VENDORID","NAME"])
        self.vendors = self.dictify(vendors,"NAME","VENDORID")
        
        # Lookup for Locations
        locations = self.client.get_entity(object_type='locations',fields=["LOCATIONID","NAME"])
